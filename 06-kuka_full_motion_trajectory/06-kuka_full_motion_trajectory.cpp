...
using namespace std;
const double time_slowdown_factor = 5;
...
void control(Sai2Model::Sai2Model* robot, Sai2Model::Sai2Model* object_model, Simulation::Sai2Simulation* sim);
...
int main (int argc, char** argv) {
	...
	auto object = new Sai2Model::Sai2Model(object_fname, false);
	...
	thread ctrl_thread(control, robot, object, sim);
    ...
    while (!glfwWindowShouldClose(window)) {
		...
		graphics->updateGraphics(object_name, object);
...
void control(Sai2Model::Sai2Model* robot, Sai2Model::Sai2Model* object_model, Simulation::Sai2Simulation* sim) {
    ...
    object_model->_q[0] = 10.660208;
    object_model->_q[1] = 11.139275;
    object_model->_q[2] = 12.639789;
//    object_model->_q[0] = 0.660208;
//    object_model->_q[1] = 0.139275;
//    object_model->_q[2] = 0.639789;
//    object_model->_q[0] = 0;
//    object_model->_q[1] = 0;
//    object_model->_q[2] = 0;
    Eigen::VectorXd zeros;
    zeros.setZero(object_model->dof());
    object_model->_dq = zeros;
    sim->setJointPositions(object_name, object_model->_q);
	...
	while (fSimulationRunning) { //automatically set to false when simulation is quit
		...
		double R = 0.3; //m
		double T = 5.0; //sec
		double xd = R*sin(2.0*M_PI*curr_time/T);
		double yd = R*cos(2.0*M_PI*curr_time/T);
		double zd = 0.3 + R/2.0 - R/2.0*cos(4.0*M_PI*curr_time/T);

		Eigen::Quaterniond lambdad (
			1/sqrt(2)*sin(M_PI/4.0*cos(2.0*M_PI*curr_time/T)),
			1/sqrt(2)*cos(M_PI/4.0*cos(2.0*M_PI*curr_time/T)),
			1/sqrt(2)*sin(M_PI/4.0*sin(2.0*M_PI*curr_time/T)),
			1/sqrt(2)*cos(M_PI/4.0*sin(2.0*M_PI*curr_time/T))
		);
//        Eigen::Quaterniond lambdad (
//            0,0,0,1
//        );

        double dxd = 2.0*M_PI*R/T*cos(2.0*M_PI*curr_time/T);
		...
		Eigen::Quaterniond dlambdad (
			...
			pow(M_PI, 2)/(2*T*sqrt(2.0))*cos(M_PI/4.0*cos(2.0*M_PI*curr_time/T))*cos(2.0*M_PI*curr_time/T),
			-pow(M_PI, 2)/(2*T*sqrt(2.0))*sin(M_PI/4.0*cos(2.0*M_PI*curr_time/T))*cos(2.0*M_PI*curr_time/T)
		);
//        Eigen::Quaterniond dlambdad (
//            0,0,0,0
//        );

        double ddxd = 4.0*M_PI*M_PI/T*R/T*sin(2.0*M_PI*curr_time/T);
		...
		Eigen::Quaterniond ddlambdad(
			...
			1/sqrt(2)*(-sin(M_PI/4.0*cos(2.0*M_PI*curr_time/T)) * pow(-pow(M_PI, 2)/(2.0*T)*cos(2.0*M_PI*curr_time/T), 2) + cos(M_PI/4.0*cos(2.0*M_PI*curr_time/T)) * pow(M_PI, 3)/(T*T)*sin(2.0*M_PI*curr_time/T)),
			1/sqrt(2)*(-cos(M_PI/4.0*cos(2.0*M_PI*curr_time/T)) * pow(-pow(M_PI, 2)/(2.0*T)*cos(2.0*M_PI*curr_time/T), 2) - sin(M_PI/4.0*cos(2.0*M_PI*curr_time/T)) * pow(M_PI, 3)/(T*T)*sin(2.0*M_PI*curr_time/T))
		);
//        Eigen::Quaterniond ddlambdad (
//            0,0,0,0
//        );
